...
if (!function_exists('create_default_teacher()')) {
    ...
    {
        $user = User::create([
            'name' => config('casteaching.default_teacher.name', 'Sergi Tur Badenas'),
            'email' => config('casteaching.default_teacher.email', 'sergiturbadenas@gmail.com'),
            'password' => Hash::make(config('casteaching.default_teacher.password', 'alumne')),
        ]);
        $user->superadmin = true;
        $user->save();
